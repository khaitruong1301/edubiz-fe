...
export default function Sider_HomeTemplate_Destop({
  isOpenFlashSale,
  collapsed,
}) {
  return (
    <>
      <Sider
        ...
        className={` z-0 bg-transparent sider_theme rounded-t-none border-t-0 `}
        width={150}
        trigger={null}
        collapsible
        collapsed={collapsed}
          ...
          {sidebarRoute.map((item, index) => {
            ...
            return item.disable ? (
              ...
              >
                <div
                  key={index}
                  className={"flex flex-col  p-2 py-3 justify-center items-center  h-20 m-0  w-full space-y-1 transform duration-300  rounded-lg  my-1 " + cssBtn}
                >
                  <button >
                    {" "}
                    {item.icon}
                  ...
                  <span
                    className={
                      collapsed
                        ...
                        : `  rounded-sm  break-words w-max text-center px-1 font-normal  `
                    }
                  >
                    {item.title}
                  </span>
