...
export default function WheelComponent({ sizeBtnStart = 50, textSize, widthContainer, heightContainer, ..._ref }) {
    ...
    var handleOpenModal = (
        ...
        if (localStorageServ.luckyWhell.get() >= 2) {
    ...
    var onTimerTick = function onTimerTick() {
        ...
        if (finished) {
            ...
            httpServ.postKetQuaLuckyWhell({
                ...
                if (res.data.content) {
                    let luckyWhellLocal = localStorageServ.luckyWhell.get()
                    if (luckyWhellLocal) {
                        localStorageServ.luckyWhell.set(luckyWhellLocal + 1)
                    } else {
                        localStorageServ.luckyWhell.set(1)
                    }
