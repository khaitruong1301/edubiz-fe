    ...
    render() {
        ...
        return (
            <div className="enter-quiz pl-5">
                ...
                <div className="enter-quiz-answer row">
                <div className="text-center col-md-12">
                        <Divider>CÂU TRẢ LỜI</Divider>
                    </div>
                    {
                        dsDapAn.map((dapAn, index) => {
                            // return <Tag key={index} color="#108ee9" onClick={() => this.onChange(dapAn)}>
                            //     {dapAn}
                            // </Tag>
                            return <div className="col-md-6"><label key={index} class="ant-radio-wrapper" onClick={() => this.onChange(dapAn)}>
                                {dapAn}
                            </label></div>
                        })
                    }
                    <div className="col-md-12 text-center py-3">
                        <button className="btn btn-info" onClick={this.onRefresh}><i className="fa fa-undo"></i></button>
                    </div>
                </div>
            </div>
        )
